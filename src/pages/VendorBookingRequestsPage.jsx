...
import { getBookingRequestForVendor, updateBookingRequestStatusForVendor } from "../utils/repository/vendor";

const VendorBookingRequestsPage = () => {
  ...
  const handleAcceptRequest = async () => {
    ...
    const res = await updateBookingRequestStatusForVendor({
      requestId: selectedRequest?.id || "",
      vendorId: user?.userDetails?.id || '',
  ...
  const fetchBookingRequests = async () => {
    setLoading(true);
    const res = await getBookingRequestForVendor(user?.userDetails?.id || "");
    let temp = [...res?.data];
    temp = temp.filter(req => {
      let vendor = (req?.selectedServices ?? []).find(service => service?.id === user?.userDetails?.id);
      if(!vendor?.vendorAccepted && !vendor?.vendorRejected){
        return true;
      }
      return false;
    });
              ...
              <div className="space-y-3 text-gray-700 text-sm sm:text-base leading-relaxed">
                ...
                <div className="h-auto">
                    ...
                    <textarea
                        rows={7}
                        value={
                            (selectedRequest?.selectedServices ?? [])?.find(item => item.id === user?.userDetails?.id)?.instructions || "No Instructions present"
                        }
                        disabled={true}
                 ...
                 <div className="h-auto">
                    <p className="font-semibold text-gray-800">Quotation Amount: </p>
                    <TextField 
                        type="number" 
                        size="small" 
                        value={amount}
